...
url_AdGuardHome="https://gitee.com/privacy-protection-tools/anti-ad/raw/master/anti-ad-for-dnsmasq.conf"
	...
	if [[ "1" == "1" ]]; then
		...
		for i in {1..5}; do
			...
			replenish_/rules_local=`cat /rules/yhosts.txt  | sed -n '2p' | cut -d "=" -f2`
			replenish_/rules_remote=`cat /tmp/yhosts.txt | sed -n '2p' | cut -d "=" -f2`
			mobile_nu_local=`grep -E -v "^!" /tmp/yhosts.txt | wc -l`
			echo_date yhosts规则本地版本号： $replenish_/rules_local
			echo_date yhosts规则在线版本号： $replenish_/rules_remote
			if [[ "$mobile_nu_local" -gt 5000 ]]; then
				if [[ "$replenish_/rules_local" != "$replenish_/rules_remote" ]]; then
					echo_date 将临时文件覆盖到原始yhosts文件
					mv /tmp/yhosts.txt /rules/yhosts.txt
					koolproxy_https_mobile=1
					break
				else
					echo_date 检测到yhosts本地版本号和在线版本号相同，那还更新个毛啊!
				fi
			else
	...
	if [[ "1" == "1" ]]; then
		...
		for i in {1..5}; do
			...
			fanboy_/rules_remote=`cat /tmp/fanboy.txt  | sed -n '3p'|awk '{print $3,$4}'`
			fanboy_nu_local=`grep -E -v "^!" /tmp/fanboy.txt | wc -l`
			echo_date fanboy规则本地版本号： $fanboy_/rules_local
			echo_date fanboy规则在线版本号： $fanboy_/rules_remote
			if [[ "$fanboy_nu_local" -gt 15000 ]]; then
				if [[ "$fanboy_/rules_local" != "$fanboy_/rules_remote" ]]; then
